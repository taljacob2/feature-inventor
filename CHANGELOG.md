...
## 2026-08-01 — Backlog section counts (Next/Later/Horizon) in `status`
- Effort/value estimate vs actual: ICE 6/7/8 (composite 7.0) — matched; generalizing `parseNowSection`'s bullet/continuation-line logic into a shared, heading-parameterized `parseSection` helper was mechanical reuse as expected, no new parsing edge cases.
- Sanity checks: pass — `npm test` (27/27 tests, including 4 new `src/roadmap.test.ts` cases for `parseBacklogCounts` and 1 new `src/cli.test.ts` case) and `npm run build` both green; manually ran `dist/cli.js status` from the repo root and confirmed a `Backlog — Next: 2, Later: 4, Horizon: 2` line matching a manual count of ROADMAP.md's actual open items in each section.
- Commit: this commit
- Notes from re-evaluation: Refactored `src/roadmap.ts` to extract a private `parseSection(roadmapMd, headingName)` helper (open-item bullet/continuation parsing, previously duplicated in `parseNowSection`) and added `parseBacklogCounts(roadmapMd)` returning `{ next, later, horizon }` counts of open items, reusing the same `\b`-bounded heading regex and fenced-code-block stripping so headings like "Next Steps" or a `## Next` line inside a code fence don't produce false matches. `printStatus` in `src/cli.ts` now prints a `Backlog — Next: N, Later: N, Horizon: N` line under "Up next".
