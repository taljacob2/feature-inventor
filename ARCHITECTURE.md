...
8. Convert any remaining runtime-specific paths, including the historical workflow script, into explicit adapters or retire them from supported execution.
9. **Foundation complete:** Add risk-aware verification policies based on explicit curated risk tags, protected paths, and operator-configured check mappings. The derived decision is immutable proposal and review metadata; richer captured-artifact interpretation remains future work.
10. **Complete boundary:** Retire the legacy daemon that invoked the archived workflow. `schedule handoff` now records an exact proposal-pinned command without scheduling or executing it.
11. **Deferred by product decision:** Keep scheduler handoffs non-executing until there is an explicit need to start approved work on a timetable. Automatic local index preparation now addresses faster context without introducing background execution, a persistent host, or a schedule.
12. Support a selected external target repository only after the shared engine is proven.
